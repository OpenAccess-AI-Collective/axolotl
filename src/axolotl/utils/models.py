...
class ModelLoader:
    def __init__(self, cfg: DictDefault,
                 ...
                 reference_model: bool = False,) -> None:
        self.cfg = cfg
    ...
    def apply_patches(self) -> None:
        ...
        if (
            ...
            patch_for_multipack(self.cfg.model_config_type, model_name=self.cfg.base_model)
        ...
        if (
            ...
            self.patch_mistral_derived_model()
    ...
    def patch_llama_derived_model(self) -> None:
        ...
        if self.cfg.sample_packing and not self.inference:
    ...
    def patch_mistral_derived_model(self) -> None:
        from axolotl.monkeypatch.mistral_attn_hijack_flash import (
            replace_mistral_attn_with_flash_attn,
        )

        LOG.info("patching mistral with flash attention")
        replace_mistral_attn_with_flash_attn(packed=self.cfg.sample_packing)
        ...
        if max_memory is not None:
            ...
            with init_empty_weights():
                model_canvas = AutoModelForCausalLM.from_config(
                    self.model_config, trust_remote_code=self.cfg.trust_remote_code or False
                )
    ...
    def set_quantization_config(self) -> None:
        ...
        if self.cfg.adapter == "qlora" and self.cfg.load_in_4bit:
            bnb_config = {
                "load_in_4bit": True,
        ...
        if (self.cfg.adapter is not None
            and not ("quantization_config" in self.model_kwargs or self.cfg.gptq)
        ):
    ...
    def set_attention_config(self) -> None:
        ...
        """
        if self.cfg.flash_attention:
            if not self.cfg.sample_packing:
                ...
                if self.model_config.model_type in SUPPORTED_MULTIPACK_MODEL_TYPES:
                    self.model_kwargs["attn_implementation"] = "flash_attention_2"
                    self.model_config._attn_implementation = (  # pylint: disable=protected-access
                        "flash_attention_2"
                    )
            ...
            self.model_config._attn_implementation = "sdpa"  # pylint: disable=protected-access
        elif self.cfg.eager_attention:
            self.model_kwargs["attn_implementation"] = "eager"
            self.model_config._attn_implementation = "eager"  # pylint: disable=protected-access

        if self.cfg.low_cpu_mem_usage:
            self.model_kwargs["low_cpu_mem_usage"] = True

    def build_model(self, qlora_fsdp) -> Tuple[PreTrainedModel, bool]:
        ...
        if (  # pylint: disable=condition-evals-to-constant)
            ...
            and self.cfg.model_config_type == "dbrx"
        ):
            ...
            if qlora_fsdp and self.cfg.fsdp_config.fsdp_cpu_ram_efficient_loading:
                skip_move_to_device = True
            ...
            if self.cfg.gptq:
                ...
                model = getattr(transformers, self.model_type).from_pretrained(
                    self.base_model,
                    config=self.model_config,
                    trust_remote_code=self.cfg.trust_remote_code or False,
                    **self.model_kwargs,
        ...
        return model, skip_move_to_device


    def ajust_model_config(self, model) -> None:
        if (
            hasattr(model, "config")
            and hasattr(model.config, "max_position_embeddings")
            and model.config.max_position_embeddings
            and self.cfg.sequence_len > model.config.max_position_embeddings
        ):
            LOG.warning(
                f"increasing model.config.max_position_embeddings from {model.config.max_position_embeddings} to {self.cfg.sequence_len}"
            )
            model.config.max_position_embeddings = self.cfg.sequence_len
    ...
    def set_z3_leaf_modules(self, model) -> None:
        ...
        elif self.cfg.model_config_type == "dbrx":
    ...
    def prepare_model(self, qlora_fsdp) -> None:
        ...
        loftq_bits = self.cfg.peft and self.cfg.peft.loftq_config and self.cfg.peft.loftq_config.loftq_bits
        if self.cfg.adapter == "lora" and loftq_bits:
            skip_prepare_model_for_kbit_training = True

        if qlora_fsdp or (self.cfg.fsdp and self.cfg.fsdp_config.fsdp_cpu_ram_efficient_loading):
            # make sure everything is in the same dtype
            skip_prepare_model_for_kbit_training = True

        if (not skip_prepare_model_for_kbit_training
            and self.cfg.adapter in ["lora", "qlora"]
            and (self.model_kwargs["load_in_8bit"] or self.model_kwargs["load_in_4bit"])
        ):
            LOG.info("converting PEFT model w/ prepare_model_for_kbit_training")
            model = prepare_model_for_kbit_training(
                model, use_gradient_checkpointing=self.cfg.gradient_checkpointing
            )
    ...
    def apply_lora_patch(self, model) -> None:
    ...
    def load_model(self) -> Tuple[PreTrainedModel, Optional[PeftConfig]]:
        ...
        try:
            model, skip_move_to_device = self.build_model(qlora_fsdp)
        ...
        if not self.cfg.fsdp:
            ...
            self.convert_embedding_modules_dtype(model, embedding_modules,
                            dist_dtype=torch.float32, before_kbit_train_or_finetune=True)

        if is_deepspeed_zero3_enabled():
            self.set_z3_leaf_modules(model)
        ...
        if (needs_fa2_dtype or self.cfg.flash_attention) and not qlora_fsdp:
            LOG.info("converting modules to %s for flash attention", self.cfg.torch_dtype)
            self.convert_embedding_modules_dtype(model, embedding_modules,
                            dist_dtype=self.cfg.torch_dtype, before_kbit_train_or_finetune=False)
        ...
        if not self.reference_model or self.cfg.lora_model_dir:
            ...
            if self.cfg.adapter and self.cfg.rl in ["dpo", "ipo", "kto"] and not self.cfg.merge_lora:
                _, lora_config = load_lora(model, self.cfg, inference=False, config_only=True)
            else:
                model, lora_config = load_adapter(model, self.cfg, self.cfg.adapter)
        ...
        if (
            ...
            model.to(f"{self.device.__str__()}:{self.cfg.local_rank}")
